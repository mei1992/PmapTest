<template>
    <div id="map_location" class="map_citypopup">
        <div class="citypopup_main">
            <div class="title"><span>当前城市: </span><span>{{cur_cityName}}</span></div>
            <div class="content" style="height: 369px; overflow: hidden;">
                ...
                <span href="javascript:void(0)" tid="selCityProvince" id="selCityProvince" class="sel-city-btnl"
                    ...
                    <div id="selCityPlaceListId" class="selCityPlaceList"
                         style="width: 100%; height: 149px; padding-bottom: 24px">
            ...
            <button id="popup_close" title="关闭" v-on:click="btnAction"></button>
        </div>
...
<style>
    ...
    .map_citypopup .content {
        height: 369px;
        overflow: hidden;
    }

    .map_citypopup button {
        position: absolute;
        z-index: 50;
        top: 10px;
        right: 10px;
    ...
    export default {
        ...
        data () {
            return {
                ...
                cityName: "北京市",//城市名字
                closeImage: globals.TOOL_CLOSE_IMG,
            }
        },
        computed: {
            ...mapGetters(['cur_cityName','cur_code'])
        },
        methods: {
            ...mapActions(['updateCurCityName','updateCurCode']),
            // 获取城市列表
            ...
            selectArea: function (item) {
                ...
                this.updateCurCityName(item.cname);
                this.updateCurCode(item.code);
                if (this.cityName === "全国") {
            ...
            //关闭按钮事件
            btnAction: function () {
                var location = document.getElementById("map_location");
        ...
        mounted () {
            ...
            $('#popup_close').css('background', 'url(' + this.closeImage + ') no-repeat -2px -3px');
        }
    }
